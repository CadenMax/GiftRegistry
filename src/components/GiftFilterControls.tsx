...
export function GiftFilterControls({ registry, filters, setFilters, claimFilter, setClaimFilter, children }: GiftFilterControlsProps) {
    ...
    return (
        <section className="filter-section">
            ...
            <label className="price-range-field">Price range<div className="price-range-control"><div className="price-range-values"><span>${minPrice}</span><span>${maxPrice}</span></div><div className="price-range-sliders"><input aria-label="Minimum price" max={maxGiftPrice} min="0" onChange={(event) => updateMinPrice(Number(event.target.value))} type="range" value={minPrice} /><input aria-label="Maximum price" max={maxGiftPrice} min="0" onChange={(event) => updateMaxPrice(Number(event.target.value))} type="range" value={maxPrice} /></div></div></label>
