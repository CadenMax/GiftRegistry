      ...
      {!unlocked ? (
        ...
        <>
          ...
          <div className="giver-summary">
            ...
            <span><i className="legend-dot available-dot" /> Still available <i className="legend-dot considering-dot" /> Being considered <i className="legend-dot claimed-dot" /> Claimed</span>
          </div>
            ...
            {view?.gifts.map((gift) => (
              <article className="gift-card" key={gift.id}>
                ...
                <div className="gift-copy">
                  <div className="gift-title-row"><div>{gift.status ? <span className="gift-status">{gift.status}</span> : null}<h3>{gift.title}</h3><p>{gift.description || "No description added."}</p></div></div>
                  <div className="gift-footer"><span>{gift.categoryName}</span><strong>{gift.priceLabel}</strong></div>
