...
const statements = {
  ...
  createNotification: database.prepare('INSERT INTO notifications (id, account_id, registry_id, gift_id, type, title, body, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'),
    ...
    guestProfiles: database.prepare('SELECT profile_id AS id, profile_token AS claimToken, display_name AS name, avatar_url AS avatarUrl, registry_id FROM guest_profiles WHERE registry_id = ? ORDER BY display_name COLLATE NOCASE ASC'),
